...
try
		...
		addon = lasterr();
		failed = failed + 4;
		s{length(s)+1}=['Failed Test 7-10: generateQP wrong ' addon];
end;
if failtest == false,
		try
		...
		if failtest,
		    failed=failed+1;
				s{length(s)+1}=['Failed Test 7: lower and upper bound not the same size ' addon];
		else
				ok=ok+1;
		end;
		fprintf('Completed Test 7\n');
		failtest = false;
		try
		...
		if failtest,
		    failed=failed+1;
				s{length(s)+1}=['Failed Test 8: number of rows of A not the same as the number of rows of b ' addon];
		else
				ok=ok+1;
		end;
		fprintf('Completed Test 8\n');
		failtest = false;
		try
		...
		if failtest,
		    failed=failed+1;
				s{length(s)+1}=['Failed Test 9: number of rows of H not the same as the number of rows of q ' addon];
		else
			ok=ok+1;
		end;
		fprintf('Completed Test 9\n');
		failtest = false;
		try
		...
		if failtest,
		    failed=failed+1;
				s{length(s)+1}=['Failed Test 10: H is incorrect ' addon];
		else
			ok=ok+1;
		end;
		fprintf('Completed Test 10\n');
end;
fprintf('Completed Test 7~10\n');

fprintf('Starting Test 11~13\n');
failtest = false;
%svmclassify test
X = rand(20,100);
w = rand(20,1);
...
try
		...
		addon = lasterr();
		failed = failed + 3;
		s{length(s)+1}=['Failed Test 11~13: Linear SVM classification using trainsvm and svmclassfy does not work ' addon];
end;
...
if failtest == false,
		...
		if failtest,
		    failed=failed+1;
				s{length(s)+1}=['Failed Test 11: Linear SVM classification is incorrect' addon];
		else
			ok=ok+1;
		end;
		fprintf('Completed Test 11\n');
		failtest = false;
		try
		...
		if failtest,
		    failed=failed+1;
				s{length(s)+1}=['Failed Test 12: alpha not greater than 0' addon];
		else
				ok=ok+1;
		end;
		fprintf('Completed Test 12\n');
		failtest = false;
		try
		...
		if failtest,
		    failed=failed+1;
				s{length(s)+1}=['Failed Test 13: some vectors in sv_i are not support vectors' addon];
		else
			ok=ok+1;
		end;
		fprintf('Completed Test 13\n');
end;
fprintf('Completed Test 11~13\n');
